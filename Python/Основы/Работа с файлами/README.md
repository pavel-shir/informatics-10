...
`'x'` | exclusive creation | Открытие на эксклюзивную запись, выбрасывает исключение если файл уже существует
...
Методом `seek()` можно двигать указатель не только относительно начала файла - для этого неоюходимо передать ему второй параметр.
...
Стоит заметить, что символ переноса попадает в прочитанные данные.
