...
Функция `input()` возвращает тип `string`:
...
Если нужно ввести число - необходимо провести преобразование типа - к примеру, через функцию `int()` (приведение к целому числу) или `float()` (приведение к числу с плавающей точкой):
...
Теперь необходимо привести каждый элемент списка к типу `int` - сделать это можно пробежав по всем элементам списка, применить к каждому функцию `int()` и записать полученные значения в новый список:
...
Данная операция выглядит несколько громоздко. В Python мы можем воспользоваться функцией `map()` если мы хотим сделать какое-то преобразование сразу со всеми элементами списка:
...
Иногда нам заранее известно количество вводимых через пробел чисел - и иногда не совсем удобно работать с ними в списке, а хочется дать каждому из них свою переменную.
...
В данном случае происходит *деструктурирование* списка - значение первого (с индексом `0`) элемента будет записано в `n`, а значение второго (с индексом `1`) будет записано в `m`. Работает это и любым количеством переменных - главное, чтобы количество переменных слева от оператора `=` и количество элементов массива было одинаковым.

В итоге можно вообще отказаться от лишней переменной, и записать пример так:
...
Разделитель можно поменять - для этого у функции `print()` есть *именованный параметр* `sep`. К примеру, если нужно использовать `'_'` вместо пробела в качестве разделителя, то код будет выглядеть так:
...
При вызове `print()` несколько раз каждый вывод будет происходить в новой строке.
...
В итоге в выводе не будет переноса строки после последней строки. На практике такой способ используется очень редко.
...
Отличие только в том, что при выводе в цикле после последнего элемента идет символ пробела, в то время как при выводе с распаковкой лишнего пробела нет.
